...
function listGifts(letter) {
  const giftsToArray = letter.split(/\s+/).filter(Boolean);
  const gifts = giftsToArray.filter((gift) => !gift.startsWith("_"));
  const giftToObject = gifts.reduce((newObject = new Object(), current) => {
    newObject.hasOwnProperty(current)
      ? newObject[current]++
      : (newObject[current] = 1);
    return newObject;
  }, {});
